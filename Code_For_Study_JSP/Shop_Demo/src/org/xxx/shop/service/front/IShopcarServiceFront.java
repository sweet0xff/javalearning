...
public interface IShopcarServiceFront {
    /**
     * 主要实现购物车的数据增加，具备如下的操作逻辑：<br>
     *     <li>首先要查询增加的购物车数据是否已经存在，如果存在则进行保存量自增</li>
     *     <li>如果要保存的信息不存在，则保持一个新的内容，并且数量是1</li>
     * @param vo
     * @return
     * @throws Exception
     */
     ...
     *     <li>购物记录包含有全部商品编号，就可以查询出全部的完整商品信息</li>
     *     <li>由于要考虑到回显问题，所以应该再让其显示出所有的购买记录</li>
     * @param mid 用户id
     * @return 包含有如下内容：<br>
    ...
    /**
     * 调用IShopcar.doRemoveByMemberAndGoods()方法，删除掉知道一个用户和商品的信息
     * @param mid
     * @param gid
     * @return
     * @throws Exception
     */
    public boolean delectCar(String mid,Set<Integer> gid)throws Exception;
