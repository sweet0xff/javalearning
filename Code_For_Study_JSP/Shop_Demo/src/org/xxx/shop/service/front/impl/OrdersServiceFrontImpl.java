...
public class OrdersServiceFrontImpl implements IOrdersServiceFront {
    ...
    public boolean insert(String mid) throws UnCompleteMemberInfomrationException, UnEnoughAmountException, EmptyShopcarException, SQLException {
        ...
        try {
            ...
            // 2、如果现在没有姓名、电话、地址那么根本不可能创建订单
            if (member.getName() == null || member.getPhone() == null || member.getAddress() == null) {
                throw new UnCompleteMemberInfomrationException("用户信息不完整，请先完善后再操作！");
            }
            // 如果现在信息完整，判断数据库中数据是否包含有购物车信息
            Map<Integer, Integer> cars = DAOFactory.getIShopcarDAOInstance(this.dbc.getConnection()).findAllByMember(mid);
            // 4、购物车中没有任何的购买商品信息
            if (cars.size() == 0) {
                throw new EmptyShopcarException("你的购物车中没有任何商品信息，无法创建订单！");
            }
            // 5、如果现在要是有购物车信息，则要查询出所有的商品信息，目的是取得商品的名称、价格、库存量
            ...
            while (iterGoods.hasNext()) {
                ...
                pay += vo.getPrice() * cars.get(vo.getGid()); // 单格 * 购买数量
            }
            // 创建订单对象
            ...
            // 9、订单创建完成之后一定要去的当前增长后的订单编号，因为订单详情需要这个编号
            ...
            flag = DAOFactory.getIDetailsDAOInstance(this.dbc.getConnection()).doCarteBath(all);
            // 12、清空购物车信息
            flag = DAOFactory.getIShopcarDAOInstance(this.dbc.getConnection()).doRemoveByMember(mid);
            dbc.getConnection().setAutoCommit(true); // 执行提交
        } catch (SQLException e) {
            dbc.getConnection().rollback(); // 有异常执行回滚
            throw e;
        } finally {
            this.dbc.close();
        }
